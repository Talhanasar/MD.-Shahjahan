...
export default function AboutPage() {
  ...
  return (
    <>
      {/* Hero / header */}
      <section className="py-16 md:py-24 bg-linear-to-br from-teal-900 via-teal-800 to-teal-900 text-white relative overflow-hidden">
        ...
        <div className="mx-auto w-full max-w-none px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-7 space-y-5">
              ...
              <p className="text-lg md:text-xl text-teal-50/90 max-w-2xl">
                {data.bio.short}
              </p>
              <p className="text-lg md:text-xl text-teal-50/90 max-w-2xl">
                {data.bio.long}
      ...
      <section className="py-16 md:py-20 bg-stone-50">
        <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 max-w-5xl space-y-12">
          {/* Biography */}
          <div className="bg-white rounded-3xl shadow-sm border border-stone-200/70 p-6 sm:p-8 md:p-10 space-y-4">
            <h2 className="text-2xl sm:text-3xl font-serif font-bold text-stone-900">
              {t.about?.biography?.title ?? data.bio.aboutTitle}
            </h2>
            <p className="text-base sm:text-lg text-stone-700 leading-relaxed">
              {t.about?.biography?.body ?? data.bio.long.join(' ')}
            </p>
            
          </div>

          {/* Family legacy */}
          <div className="rounded-3xl bg-linear-to-br from-teal-50 to-teal-50 border border-teal-100 p-6 sm:p-8 md:p-10 space-y-5">
            <h2 className="text-2xl sm:text-3xl font-serif font-bold text-teal-900">
              {t.about?.familyLegacy?.title ?? "Family Legacy"}
            </h2>
            <ul className="space-y-3 text-base sm:text-lg text-stone-800">
              {(t.about?.familyLegacy?.items ?? []).map((item: string, idx: number) => (
            ...
            <div className="md:col-span-1 bg-stone-900 text-white rounded-3xl p-6 sm:p-7 flex flex-col justify-between">
              <div>
                ...
                <p className="mt-2 text-sm sm:text-base text-stone-200">
                  {t.about?.experienceLabel ?? data.experience.label}
                </p>
              </div>
            ...
            <div className="md:col-span-2 bg-white rounded-3xl p-6 sm:p-8 border border-stone-200/70">
              ...
              <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
                <ul className="space-y-3">
                  {(t.about?.skills?.left ?? data.skills.left).map((skill: string, idx: number) => (
                ...
                <ul className="space-y-3">
                  {(t.about?.skills?.right ?? data.skills.right).map((skill: string, idx: number) => (
                    <li key={idx} className="flex items-center gap-3 text-stone-700">
                      <span className="w-6 h-6 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 shrink-0">
                        <Check className="w-3 h-3" />
                      </span>
